...
@router.message()
async def check_ban_middleware(message: Message, handler):
    if await check_user_banned(message.from_user.id):
        await message.answer(f"{em('cross')} Вы заблокированы!")
        return
    return await handler(message)

@router.callback_query()
async def check_ban_callback(callback: CallbackQuery, handler):
    if await check_user_banned(callback.from_user.id):
...
async def search_len_handler(callback: CallbackQuery):
    ...
    # Проверка дневного лимита для 5 букв
    if length == 5:
...
async def admin_create_promo_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"{em('gift')} <b>Введи код промокода (например: WELCOME50):</b>",
        reply_markup=get_back_button()
    )
...
async def admin_reset_limits(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        f"{em('check')} <b>Лимиты сброшены!</b>",
        reply_markup=get_admin_keyboard()
    )
