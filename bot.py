...
# --- Премиум эмодзи (для инлайн кнопок и сообщений) ---
EMOJI = {
...
# --- Обычные эмодзи для ReplyKeyboard (премиум не работают в обычных кнопках) ---
def get_main_keyboard():
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="🔍 Поиск"), KeyboardButton(text="📦 Маркет")],
            [KeyboardButton(text="👤 Профиль")]
        ],
        resize_keyboard=True
    )

# --- Быстрая проверка юзернейма (параллельно) ---
...
async def check_many_usernames(usernames: List[str], max_workers: int = 20) -> List[str]:
...
# --- Генерация юзернеймов для поиска ---
def generate_by_length(length: int, count: int = 30) -> List[str]:
    """Генерирует случайные юзернеймы заданной длины"""
    variants = set()
...
def generate_with_keyword(keyword: str, length: int, count: int = 30) -> List[str]:
    """Генерирует варианты с ключевым словом"""
    variants = set()
    alphabet = string.ascii_lowercase
    digits = string.digits
    
    # Вариант 1: ключевое слово в начале
    if len(keyword) < length:
        suffix_len = length - len(keyword)
        for _ in range(count // 3):
            suffix = ''.join(random.choices(alphabet + digits, k=suffix_len))
            variants.add(keyword + suffix)
    
    # Вариант 2: ключевое слово в конце
    if len(keyword) < length:
        prefix_len = length - len(keyword)
        for _ in range(count // 3):
            prefix = ''.join(random.choices(alphabet, k=prefix_len))
            variants.add(prefix + keyword)
    
    # Вариант 3: ключевое слово внутри
    if len(keyword) + 2 <= length:
        remaining = length - len(keyword)
        for _ in range(count // 3):
            left = random.randint(1, remaining - 1)
            right = remaining - left
            prefix = ''.join(random.choices(alphabet, k=left))
            suffix = ''.join(random.choices(alphabet + digits, k=right))
            variants.add(prefix + keyword + suffix)
    
    # Вариант 4: leet замены
    leet_map = {'a': '4', 'e': '3', 'i': '1', 'o': '0', 's': '5'}
    leet_keyword = ''.join(leet_map.get(c, c) for c in keyword)
    if leet_keyword != keyword and len(leet_keyword) <= length:
        variants.add(leet_keyword)
        if len(leet_keyword) < length:
            variants.add(leet_keyword + ''.join(random.choices(digits, k=length - len(leet_keyword))))
...
@router.message(F.text.in_(["🔍 Поиск", f"{em('search')} Поиск"]))
...
@router.message(F.text.in_(["👤 Профиль", f"{em('profile')} Профиль"]))
...
@router.message(F.text.in_(["📦 Маркет", f"{em('market')} Маркет"]))
...
async def search_len_handler(callback: CallbackQuery):
    ...
    # Генерируем 30 вариантов и проверяем параллельно
    variants = generate_by_length(length, 30)
    found = await check_many_usernames(variants)
    
    if found:
        text = f"{em('check')} <b>Найдены свободные юзернеймы ({length} букв):</b>\n\n"
        text += "\n".join([f"• @{u} → t.me/{u}" for u in found[:5]])
    else:
...
async def process_length_and_search(message: Message, state: FSMContext):
    ...
    # Генерируем и проверяем параллельно
