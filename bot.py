...
from telethon.tl.functions.messages import ImportChatInviteRequest, CheckChatInviteRequest, SendMessageRequest
...
def generate_bot_usernames(keyword: str, length: int, suffix_type: str, count: int = 50) -> List[str]:
    """Генерирует юзернеймы для ботов. length - ПОЛНАЯ длина включая bot/_bot"""
    variants = set()
    letters = "abcdefghijklmnopqrstuvwxyz"
    digits = "0123456789"
...
# --- Поиск обычных юзернеймов (3 за раз) ---
...
# --- Поиск ботов (исправлено: полная длина включает bot/_bot) ---
...
async def create_handler(callback: CallbackQuery, state: FSMContext):
    ...
    if create_type == "bot":
        ...
        await callback.message.edit_text(
            ...
            f"Создано: {user['bots_created']}/{limits['bots']}\n\n"
        ...
        if user[f'{create_type}s_created'] >= limits[create_type]:
...
async def create_days(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"Это займет примерно {count * 15} секунд."
    )
    ...
    if created:
        ...
        for link in created:
            text += f"• {link}\n"
        text += f"\n{em('clock')} Каналы будут переданы тебе через {days} дней."
    else:
...
# --- Маркет (оставлено для совместимости) ---
...
async def autoreserve_count(message: Message, state: FSMContext):
    ...
    if len(found) < count:
        await message.answer(f"{em('cross')} Найдено только {len(found)}.")
        await state.clear()
        return
    ...
    for username in to_reserve:
        try:
            result = await client(CreateChannelRequest(title=title, about="Vest Search", megagroup=False))
            channel = result.chats[0]
            await client(UpdateUsernameRequest(channel, username))
            link = f"https://t.me/{username}"
