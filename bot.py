...
nft_monitoring_tasks: Dict[int, asyncio.Task] = {}
nft_cache: Dict[str, List[Dict]] = {}  # Кеш для отслеживания новых NFT
...
async def delete_user(user_id: int):
    async with db_pool.acquire() as conn:
        ...
        await conn.execute("DELETE FROM reserved_channels WHERE user_id = $1", user_id)
        await conn.execute("DELETE FROM reserved_groups WHERE user_id = $1", user_id)
...
async def get_all_nft_monitoring() -> List[asyncpg.Record]:
    async with db_pool.acquire() as conn:
        return await conn.fetch(
            """SELECT nm.*, u.user_id as uid 
               FROM nft_monitoring nm 
...
async def fetch_nft_collections() -> List[Dict]:
    ...
    collections = [
        ...
        {"name": "Telegram Usernames", "address": "EQCt9Z3jF2mX8hY5nL1kP7rS6wV0xT4yB8aD9cE2fG3h", "description": "Fragment юзернеймы"},
        {"name": "Anonymous Telegram Numbers", "address": "EQDk3L4mN6oP8qR1sT5uV7wX9yZ0aB2cD4eF6gH8iJ0", "description": "Анонимные номера"},
    ]
    
    # Пытаемся получить актуальные коллекции через API Fragment
    try:
        async with http_session.get("https://fragment.com/api?hash=popular") as resp:
            if resp.status == 200:
                data = await resp.json()
                # Обработка данных от Fragment
                if "data" in data:
                    # Обновляем список популярных коллекций
                    pass
    except Exception as e:
...
async def fetch_nft_from_getgems(collection_address: str, limit: int = 10) -> List[Dict]:
    ...
    try:
        # API Getgems для получения NFT коллекции
        url = f"https://api.getgems.io/graphql"
...
async def fetch_nft_from_fragment(collection_type: str = "usernames") -> List[Dict]:
    """Получает NFT с Fragment (юзернеймы, номера)"""
    nfts = []
    
    try:
        # API Fragment для получения аукционов
            ...
            if resp.status == 200:
                ...
                for item in data.get("items", []):
                    nfts.append({
                        ...
                        "collection": "Telegram " + collection_type.title(),
                        "owner": item.get("seller", "Unknown"),
                        "price": item.get("price"),
                        "link": f"https://fragment.com/{collection_type}/{item.get('name', '')}"
                    })
    except Exception as e:
        logger.error(f"Ошибка Fragment API: {e}")
...
async def check_nft_updates(collection_address: str, collection_name: str, last_nft_id: Optional[str] = None) -> Tuple[List[Dict], str]:
    ...
    if "EQC" in collection_address or "EQD" in collection_address:
        # Getgems коллекция
        nfts = await fetch_nft_from_getgems(collection_address)
    else:
        # Fragment коллекция
        if "username" in collection_name.lower():
            nfts = await fetch_nft_from_fragment("usernames")
        else:
            nfts = await fetch_nft_from_fragment("numbers")
    
    if nfts:
        # Находим новые NFT (которых не было в прошлый раз)
        if last_nft_id:
            found_last = False
            for nft in nfts:
            ...
            # Первый запуск - запоминаем последний, но не уведомляем
            new_nfts = []
        
        # Обновляем ID последнего NFT
        if nfts:
        ...
        try:
            ...
            for mon in monitorings:
                try:
                    ...
                    # Проверяем подписку
                    if not await has_nft_monitoring(user_id):
                        continue
                    ...
                    # Отправляем уведомления о новых NFT
                    for nft in new_nfts:
                    ...
                    # Пауза между проверками разных пользователей
...
def get_main_keyboard():
    return ReplyKeyboardMarkup(
        keyboard=[
            ...
            [
                KeyboardButton(text="Создание бота", icon_custom_emoji_id=EMOJI["bot_create"]),
                KeyboardButton(text="NFT Мониторинг", icon_custom_emoji_id=EMOJI["nft"])
            ],
            [
...
def get_collections_keyboard(collections: List[Dict], page: int = 0):
    ...
    # Показываем по 5 коллекций на странице
    start = page * 5
    end = start + 5
...
@router.message(F.text == "Создание бота")
...
@router.message(F.text == "NFT Мониторинг")
...
async def menu_profile(message: Message):
    ...
    # Получаем информацию о подписке на NFT
