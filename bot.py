...
if not CRYPTO_BOT_TOKEN:
    raise ValueError("Не указан CRYPTO_BOT_TOKEN")
...
async def create_crypto_invoice(user_id: int, amount_rub: float, invoice_type: str = "deposit") -> Optional[Dict]:
    ...
    try:
        async with http_session.post(url, headers=headers, json=data) as resp:
            ...
            logger.info(f"Crypto Bot response: {result}")
...
# --- Главное меню (ТОЛЬКО ПРЕМИУМ ЭМОДЗИ) ---
...
async def process_deposit_amount(message: Message, state: FSMContext):
    ...
    if not invoice:
        await message.answer(f"{em('cross')} Ошибка создания счета. Попробуй позже.")
        await state.clear()
        return
...
async def check_payment(callback: CallbackQuery, state: FSMContext):
    ...
    if is_paid:
        ...
        await callback.message.edit_text(
            f"{em('check')} <b>Баланс пополнен на {amount} RUB!</b>",
            reply_markup=get_back_button()
        )
        await state.clear()
    else:
...
async def check_owner(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        f"Цена: {price} RUB\n"
        f"Ты получишь: {price * 0.9:.2f} RUB\n\n"
...
async def admin_balance_user(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"Текущий баланс: {float(user['balance']):.2f} RUB\n\n"
...
async def admin_stats(callback: CallbackQuery):
    ...
    text = (
        ...
        f"💰 Общий баланс: {stats['total_balance']:.2f} RUB\n"
