    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.execute("""
            CREATE TABLE IF NOT EXISTS market_listings (
                ...
                seller_joined BOOLEAN DEFAULT 0,
                buyer_joined BOOLEAN DEFAULT 0,
                buyer_id INTEGER,
...
async def market_sell_channel(message: Message, state: FSMContext):
    ...
    try:
        ...
        bot_joined = False
        logger.error(f"Ошибка вступления в канал {username}: {e}")
        ...
        try:
            await db.execute(
                """INSERT INTO market_listings 
                   (seller_id, seller_username, channel_username, price, status, bot_joined, bot_account_username, created_date) 
                   VALUES (?, ?, ?, ?, 'active', ?, ?, ?)""",
                (message.from_user.id, message.from_user.username, username, price, bot_joined, bot_username, datetime.now().isoformat())
            )
            await db.commit()
    ...
    await message.answer(
        ...
        f"{em('info')} Бот вступил в канал: {'✅' if bot_joined else '❌'}\n"
        f"Ожидай покупателя!",
        reply_markup=get_main_keyboard()
    )
    await state.clear()
