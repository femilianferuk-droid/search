...
async def check_channel_owner(client: TelegramClient, channel_username: str) -> bool:
    """Проверяет, является ли аккаунт владельцем канала"""
    try:
        ...
        if isinstance(participant.participant, (ChannelParticipantCreator, ChannelParticipantAdmin)):
            ...
            if admin_rights and admin_rights.post_messages and admin_rights.edit_messages:
                return True
...
async def buy_channel_confirm(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        f"Юзернейм: @{username}\n"
        f"Цена: {price}₽\n\n"
        f"{em('info')} Канал передан тебе. Вступи в него:\n"
...
async def market_sell_price(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"{em('link')} <b>Отправь юзернейм канала (без @):</b>",
        reply_markup=get_back_button()
    )
    await state.set_state(MarketSellStates.waiting_for_channel)

@router.message(MarketSellStates.waiting_for_channel)
async def market_sell_channel(message: Message, state: FSMContext):
    ...
    if is_free:
        await message.answer(
            f"{em('cross')} Этот юзернейм свободен! Продавать можно только существующие каналы.",
            reply_markup=get_main_keyboard()
        )
        await state.clear()
        return
    ...
    try:
        await client(InviteToChannelRequest(username, [bot_username]))
        bot_joined = True
        await message.answer(f"{em('check')} Бот @{bot_username} вступил в канал!")
    except Exception as e:
        bot_joined = False
        await message.answer(f"{em('cross')} Не удалось вступить в канал. Убедись, что канал публичный.")
        await state.clear()
        return
    ...
    await message.answer(
        f"{em('owner')} <b>Передай владельца канала</b>\n\n"
        f"1. Открой канал @{username}\n"
        f"2. Зайди в «Администраторы»\n"
        f"3. Добавь @{bot_username} как администратора\n"
        f"4. Выдай права: публикация и редактирование\n\n"
...
async def check_owner(callback: CallbackQuery, state: FSMContext):
    ...
    is_owner = await check_channel_owner(client, username)
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            """INSERT INTO market_listings 
               ...
               VALUES (?, ?, ?, ?, 'active', ?, ?, ?, ?)""",
            (callback.from_user.id, callback.from_user.username, username, price, True, True, bot_username, datetime.now().isoformat())
        )
        await db.commit()
